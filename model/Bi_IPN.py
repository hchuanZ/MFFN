# 首先我们来测试一下pool的stride
# 对于PLM，可以早期用大核+avg pool， 后期用小核+max pool
...
class Bi_IPN(nn.Module):
    def __init__(self, num_class=2, in_cube=80, use_projection=True, fusion_net='unet', task='RV', plm_nums=3, use_res=False):
        ...
        self.fine = fine_IPN(in_cube=in_cube, plm_nums=plm_nums, in_ch=slice, slices=slice, use_res=use_res)
        if use_projection:
            im_ch = 3
        else:
            im_ch = 2
